...
class AchievementSystem {
    constructor() {
        ...
        // Track last score for achievement detection
        this.lastScore = 0;
    ...
    hookIntoGame() {
        ...
        const checkGame = () => {
            if (window.game) {
                ...
                this.lastScore = window.game.score || 0;
                console.log('Achievement System: Initialized with score:', this.lastScore);
    ...
    attachGameHooks() {
        // Hook into question answering via confirmAnswer method
        const originalConfirmAnswer = window.game.confirmAnswer;
        if (originalConfirmAnswer) {
            window.game.confirmAnswer = (...args) => {
                // Store the selected answers before calling original method
                const selectedAnswers = window.game.selectedAnswerIndices ? [...window.game.selectedAnswerIndices] : [];
                
                // Call original method
                const result = originalConfirmAnswer.apply(window.game, args);
                
                // Determine if answer was correct by checking score change
                const scoreAfter = window.game.score;
                const isCorrect = scoreAfter > this.lastScore;
                this.lastScore = scoreAfter;
                
                // Call achievement handler
                this.onQuestionAnswered(selectedAnswers, isCorrect);
                
                return result;
            };
        } else {
            console.warn('Achievement System: confirmAnswer method not found, trying alternative approach');
            // Alternative: Hook directly into Game class prototype if method not yet available
    ...
    hookViaPrototype() {
        // Try to hook via Game prototype if instance method not available
        if (window.Game && window.Game.prototype && window.Game.prototype.confirmAnswer) {
            const originalConfirmAnswer = window.Game.prototype.confirmAnswer;
            const self = this;
            
            window.Game.prototype.confirmAnswer = function(...args) {
                // Store the selected answers before calling original method
                const selectedAnswers = this.selectedAnswerIndices ? [...this.selectedAnswerIndices] : [];
                
                // Call original method
                const result = originalConfirmAnswer.apply(this, args);
                
                // Determine if answer was correct by checking score change
                const scoreAfter = this.score;
                const isCorrect = scoreAfter > self.lastScore;
                self.lastScore = scoreAfter;
                
                // Call achievement handler
                self.onQuestionAnswered(selectedAnswers, isCorrect);
                
                return result;
            };
            
            console.log('Achievement System: Hooked via Game prototype');
        } else {
            console.warn('Achievement System: Could not hook into Game methods');
        }
    }
    
    onQuestionAnswered(selectedAnswers, isCorrect) {
        console.log('🏆 Achievement System: Question answered', {
            selectedAnswers,
            isCorrect,
            currentStreak: this.statistics.currentStreak,
            questionsAnswered: this.statistics.questionsAnswered
        });
        ...
        if (isCorrect) {
            ...
            console.log('❌ Wrong answer, resetting streak from', this.statistics.currentStreak, 'to 0');
    ...
    createAchievementUI() {
        ...
        if (topBar) {
            ...
            console.log('🏆 Achievement button added to top bar');
        } else {
            console.warn('🏆 Top bar not found, achievement button not added');
        }
    ...
    saveProgress() {
        ...
        console.log('💾 Achievement progress saved:', {
            questionsAnswered: this.statistics.questionsAnswered,
            questionsCorrect: this.statistics.questionsCorrect,
            currentStreak: this.statistics.currentStreak,
            bestStreak: this.statistics.bestStreak
        });
    }
    ...
    loadProgress() {
        ...
        if (saved) {
            ...
            console.log('📂 No saved achievement progress found, starting fresh');
        }
    }
    ...
    // Test method to verify achievement system functionality
    testAchievementSystem() {
        console.log('🧪 Testing Achievement System...');
        
        // Test first question achievement
        ...
        console.log('🧪 Test completed. Check console for achievement logs.');
