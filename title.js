...
class TitleScreen {
    ...
    drawTitleGlow() {
        // Animierter Glow-Effekt um den Titel (zentriert um höher gesetzten Titel)
        const glowSize = 50 + this.titleGlow * 30;
        const gradient = this.ctx.createRadialGradient(
            this.canvas.width / 2, 170, 0,
            this.canvas.width / 2, 170, glowSize
        );
        gradient.addColorStop(0, `rgba(255, 215, 0, ${this.titleGlow * 0.3})`);
        gradient.addColorStop(1, 'rgba(255, 215, 0, 0)');
        
        this.ctx.fillStyle = gradient;
        this.ctx.fillRect(0, 70, this.canvas.width, 240);
    }
    ...
    drawTitleText() {
        ...
        // Haupttitel (höher positioniert für bessere Sichtbarkeit - Canvas ist 450px hoch)
        this.ctx.font = 'bold 52px "Courier New", monospace';
        this.ctx.fillStyle = '#FFD700';
        this.ctx.textAlign = 'center';
        this.ctx.textBaseline = 'middle';
        this.ctx.fillText('KI-Manager', this.canvas.width / 2, 100);
        
        // Untertitel (entsprechend höher positioniert)
        this.ctx.font = 'bold 26px "Courier New", monospace';
        this.ctx.fillStyle = '#FFA500';
        this.ctx.fillText('ist KEIN Ponyhof!', this.canvas.width / 2, 140);
        
        // Zusätzliche Glitzer-Effekte am Text (an höhere Position angepasst)
    ...
    waitForGameAndStart() {
        console.log('Warte auf Game-Initialisierung...', !!window.game);
        
        if (window.game && typeof window.game === 'object' && window.game.gameState !== undefined) {
            console.log('Game gefunden, starte Spiel...');
            ...
            if (gameCanvas) {
                ...
                console.log('Canvas sichtbar gemacht');
            }
            if (ui) {
                ui.style.display = 'block';
                console.log('UI sichtbar gemacht');
            }
            if (inventory) {
                inventory.style.display = 'grid';
                console.log('Inventar sichtbar gemacht');
            }
            if (leftPanel) {
                leftPanel.style.display = 'block';
                console.log('Left Panel sichtbar gemacht');
            }
            
            // Starte Game Loop falls nötig
            if (window.game.startGameLoop && typeof window.game.startGameLoop === 'function') {
                window.game.startGameLoop();
                console.log('Game Loop gestartet');
            }
            
            // Starte Ingame-Musik
            if (window.game.startGameMusic && typeof window.game.startGameMusic === 'function') {
                window.game.startGameMusic();
                console.log('Spiel-Musik gestartet');
            }
            
            console.log('Spiel erfolgreich gestartet!');
        } else {
            // Warte bis Game geladen ist, mit Timeout-Schutz
            let waitCount = this.waitCount || 0;
            this.waitCount = waitCount + 1;
            
            if (waitCount < 50) { // Maximal 5 Sekunden warten (50 * 100ms)
                console.log(`Warte auf Game... Versuch ${waitCount}/50`);
                setTimeout(() => this.waitForGameAndStart(), 100);
            } else {
                console.error('Game konnte nicht geladen werden nach 5 Sekunden!');
                // Fallback: Zeige Elemente trotzdem an
                this.showElementsAsFallback();
            }
        }
    }
    
    showElementsAsFallback() {
        console.log('Fallback: Zeige Spielelemente ohne Game-Objekt an');
        const gameCanvas = document.getElementById('gameCanvas');
        const ui = document.getElementById('ui');
        const inventory = document.getElementById('inventory');
        const leftPanel = document.getElementById('leftPanel');
        ...
        // Versuche Game manuell zu erstellen
        if (!window.game && window.Game) {
            console.log('Versuche Game manuell zu erstellen...');
            try {
                window.game = new window.Game();
                console.log('Game manuell erstellt!');
            } catch(error) {
                console.error('Fehler beim manuellen Erstellen des Games:', error);
            }
        }
    }
