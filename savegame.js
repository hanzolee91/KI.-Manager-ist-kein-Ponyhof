...
class SaveGameSystem {
    ...
    collectGameData() {
        const data = {
            ...
            // Spezial-NPC System
            usedSpezialNpcs: Array.from(window.game?.usedSpezialNpcs || []),
            activeBonusEffects: this.serializeBonusEffects(window.game?.activeBonusEffects),
    ...
    serializeBonusEffects(bonusEffectsMap) {
        if (!bonusEffectsMap || !(bonusEffectsMap instanceof Map)) {
            return [];
        }
        
        return Array.from(bonusEffectsMap.entries()).map(([key, value]) => ({
            key,
            value
        }));
    }
    
    deserializeBonusEffects(serializedEffects) {
        if (!Array.isArray(serializedEffects)) {
            return new Map();
        }
        
        const bonusMap = new Map();
        serializedEffects.forEach(({key, value}) => {
            bonusMap.set(key, value);
        });
    ...
    loadFromSlot(slotNumber) {
        try {
            ...
            // Additional check for corrupted/empty save data
            if (saveData.trim() === '' || saveData === 'null' || saveData === 'undefined') {
                this.showSaveNotification(`❌ Spielstand in Slot ${slotNumber} ist beschädigt oder leer!`, 'error');
                return false;
            }
            ...
            try {
                ...
                this.showSaveNotification(`❌ Fehler beim Laden: ${parseError.message}`, 'error');
                return false;
            }
            
            if (!data || typeof data !== 'object') {
                this.showSaveNotification(`❌ Ungültiger Spielstand in Slot ${slotNumber}!`, 'error');
                return false;
            }
    ...
    restoreGameState(data) {
        try {
            ...
            if (window.game) {
                ...
                // Restore Spezial-NPC data
                window.game.usedSpezialNpcs = new Set(data.usedSpezialNpcs || []);
                window.game.activeBonusEffects = this.deserializeBonusEffects(data.activeBonusEffects || []);
